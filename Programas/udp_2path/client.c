...
void print(char *filename)
{
    ...
    for(count=0; count<count_send; count++) {
        ...
        fprintf(file_log, "%d; %d; %.0f; %ld\n", activepath[count], path[count], rtt[path[count]][count], (1000000*sendtime[count].tv_sec+sendtime[count].tv_usec));
    }
...
void initialize(char **argv)
{
	...
	int r = 1000000*(rand() % 10) + (rand()%1000000);
	usleep(r);
...
int main(int argc, char* argv[])
{
    ...
    while((count_recv+count_lost) < MAX_LIMIT) {
        ...
        else {   // Calculate Select() Timeout based on SINTERVAL
            struct timeval atual;
            gettimeofday(&atual, NULL);
            if(((count_send+2) % HBINTERVAL) != 0) {
        ...
        if(count_send < MAX_LIMIT) {    // Send Packet
            send_func(activepath[count_send], server[activepath[count_send]]); // Send Packet
            count_send++;
        ...
        if (((count_send % HBINTERVAL) == 0) && (count_send < MAX_LIMIT)) {      // Send Heartbeat each 5 packets
            ...
            send_func(hbpath, server[hbpath]);
        ...
        do {
            ...
            if (ret == -1)      // Select() Error
                ...
                if (rtt[recvpath][recvmsg] != -1) {     // Only count if timeout has not been expired
                    ...
                    if(first_recv[0]==0 && first_recv[1]==0) {  // Only changes Active Path when both paths has been tested
