...
int main(void)
{
    ...
    memset(&heartbeat, 0, sizeof(struct sctp_paddrparams)); // Zera as estruturas para manter os valores padrões para os que não forem alterados
	...
	heartbeat.spp_flags = SPP_HB_ENABLE;	// Para que as alterações abaixo tenham efeito
	...
	for (cont=1;;cont++) {
        ...
        if (cont==120)
            if(setsockopt(sfd, SOL_SCTP, SCTP_PRIMARY_ADDR, &end_prim, sizeof(end_prim)) == -1) //Altera o Endereço Primário depois de 2 minutos
