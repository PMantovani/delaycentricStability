...
{
    int     pathiter;
    float   srtts[NUMPATHS], srttl[NUMPATHS];
    ...
    struct timeval hb_tg[MAX_LIMIT]; // Initialized with TG below
    ...
    while((count_recv+count_lost) < MAX_LIMIT) {
        ...
        for (pathiter=0; pathiter<NUMPATHS; pathiter++) {
            if (flag_tg[pathiter] == 1) {
                if ((usec_diff(sendtime[count_send-1], hb_tg[pathiter]) > HB_REDUC) && (count_send < MAX_LIMIT)) {
                    send_func(pathiter, server[pathiter], HBLENGHT);

                    count_send++;
                    activepath[count_send] = activepath[count_send-1];

                    gettimeofday(&hb_tg[pathiter], NULL);
                }
            }
        }
            ...
            if (ret == -1)      // Select() Error
                ...
                if (rtt[recvpath][recvmsg] != -1) {     // Only count if timeout has not been expired
                    ...
                    if (first_recv[recvpath]) {     // First SRTT Calculus
                        srtts[recvpath] = srttl[recvpath] = rtt[recvpath][recvmsg];
                    ...
                    if(first_recv[0]==0 && first_recv[1]==0 && first_recv[2]==0) {  // Only changes path if received packets from both paths -- NOT COMPATIBLE WITH PATH EXPANSION
                        for (pathiter=0;pathiter<NUMPATHS;pathiter++) {
                            if (activepath[count_send] != pathiter) {
                                float d = (srtts[activepath[count_send]]-srttl[activepath[count_send]]) - (srtts[pathiter]-srttl[pathiter]);

                                if (d>0 && (srtts[activepath[count_send]]>THRESHOLD) && (srtts[activepath[count_send]] > srtts[pathiter])) {
