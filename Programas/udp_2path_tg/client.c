...
int main(int argc, char* argv[])
{
    ...
    while((count_recv+count_lost) < MAX_LIMIT) {
        ...
        if ((count_send >= MAX_LIMIT) && (flag_tg == 0)) {
        	gettimeofday(&endind, NULL);
        	flag_end = 1;
        }
            ...
            if (ret == -1)      // Select() Error
                ...
                if (rtt[recvpath][recvmsg] != -1) {     // Only count if timeout has not been expired
                    ...
                    if(first_recv[0]==0 && first_recv[1]==0) {  // Only changes path if received packets from both pathes
                        if ((srtt[0] >= srtt[1] + HISTERESIS) && (activepath[count_send] == 0) && (flag_tg == 0)) {    // Uses HISTERESIS to avoid multiple path changes
                            flag_tg = 1;
                            HBINTERVAL = 40000;
                            gettimeofday(&tg_time, NULL);
                        }
                        else if ((srtt[1] >= srtt[0] + HISTERESIS) && (activepath[count_send] == 1) && (flag_tg == 0)) {
                            flag_tg = 1;
                            HBINTERVAL = 40000;
                            gettimeofday(&tg_time, NULL);
                        }
        ...
        if (flag_tg == 1) {
            if (usec_diff(temp_timeout, tg_time) >= TG) {
                ...
                if((activepath[count_send] == 0) && (srtt[0] >= srtt[1] + HISTERESIS)) {
                    activepath[count_send] = 1;
                }
                else if((activepath[count_send] == 1) && (srtt[1] >= srtt[0] + HISTERESIS)) {
